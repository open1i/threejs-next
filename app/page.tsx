    ...
    >
      ...
      <div className="absolute top-4 right-4 z-10 flex gap-2">
        ...
        <Button
          ...
          className="bg-white/10 border-white/20 text-white hover:bg-white/20"
          // title={autoRotate ? "停止旋转" : "开始旋转"} // 添加提示文本
        >
          {/* <RotateCcw
            className={`h-4 w-4 ${autoRotate ? "animate-spin" : ""}`} // 添加动画效果
          /> */}
          <RotateCcw
            className={`h-4 w-4 `} // 添加动画效果
          />
        </Button>
      </div>
      ...
      >
        <Suspense
          fallback={
            <Html center>
              <div className="text-white text-xl animate-fade-in bg-black/30 backdrop-blur-sm px-6 py-4 rounded-lg shadow-lg">
        ...
        >
          {/* 环境和背景 */}
          <Environment
            preset="sunset" // 使用内置预设：sunset, night, warehouse, park, lobby
            background // 显示环境背景
            blur={0.8} // 添加模糊效果
          />
          <Stars
          ...
          <OrbitControls
            ...
            autoRotateSpeed={1} // 增加旋转速度
            minDistance={10}
            maxDistance={40}
            minPolarAngle={Math.PI / 4} // 调整垂直旋转范围
